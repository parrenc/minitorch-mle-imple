...
def logsoftmax(input: Tensor, dim: int) -> Tensor:
    ...
    '''
    max_i = max(input, dim)
    curr = (input - max_i).exp().sum(dim).log()
    return input - max_i - curr
    '''
    max_i = max(input, dim)
    return input - max_i - (input - max_i).exp().sum(dim).log()
...
def dropout(input: Tensor, rate: float, ignore: bool = False) -> Tensor:
    ...
    # TODO: Implement for Task 4.4.
    '''
    return input if ignore else input * (rand(input.shape) > rate)
    '''
    return input if ignore else (rand(input.shape) > rate) * input
